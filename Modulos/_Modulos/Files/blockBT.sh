...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/Razhiel2019/VPS-MX/master/VPS-MX_Oficial/SCRIPT/blockBT.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
...
msg -bar
echo  -e "$(msg -tit) " 
echo -e "  Panel de Firewall VPS•MX By @Razhiel ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}"
msg -bar2
